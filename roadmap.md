...
- **119A-4 (PRIORITARIA, justificada 20-09 por incidente real):** build Rust fuera de la VPS (local/CI + registry privado, la VPS solo hace pull). Datos: reinicio de dockerd en ventana de build `--no-cache` 9 min (load 5.9 en 4 cores), caída total 11 sitios; segundo build (kamples) sobrevivió pero confirma el riesgo. Alcance: registry + credenciales, compose `build:`→`image:`, flujo pull en `deploy-service`, rollback adaptado; probar la nueva ruta con otro site desechable antes de tocar producción.
