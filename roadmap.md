...
- **119A-2 (en curso, Fase A HECHA 20-09):** F3 del plan monolito + 5 helpers 100-185ef. Plan: `Agente/planes/plan-119A-2-deploy-service-2026-09-20.md`. Fase A (local, autorizada 20-09): `intentar_rollback`→R0+R3/R4/R5, `inject_postgres_data_volume`→3 pasadas, `validate_compose_before_deploy`→E4/E16/E17/E18/E19, `ensure_postgres_auth_and_hostname`→resolver/verificar/alinear, `install_rust_public_autoheal`→generar_script; check OK, test --lib 180/180, re-análisis 40→35W (5 funcion-larga deploy_service a 0, E:0). Fase B (cm-test-119a2 + E11/B0 + limpieza, AUTORIZADA 20-09 pero pendiente de re-revisión de seguridad explícita antes de cualquier operación remota: sin accidentes ni caídas en los otros 11 sitios). (20-09 pm) B0 + `delete-site` IMPLEMENTADOS (commits `0f108ac`, `efdf505`; test --lib 184/184; endpoint DELETE verificado contra docs oficiales); B2 incluirá monitoreo CPU/RAM/disco en VPS para decidir 119A-4 con datos.
...
- **119A-4 (pendiente, propuesta usuario 20-09):** build Rust fuera de la VPS (local/CI + registry privado, la VPS solo hace pull) para no sobrecargar el host productivo. Decisión pendiente de los datos de 119A-2/B2 (monitoreo CPU/RAM/disco durante el build del test-site). Si se adopta: registry + credenciales, compose `build:`→`image:`, flujo pull en `deploy-service`, rollback adaptado; probar la nueva ruta con otro site desechable antes de tocar producción. NO adelantar a 119A-2 (el test debe validar la ruta actual que usa producción).
