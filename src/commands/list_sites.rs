...
pub async fn execute(config_path: &Path, detailed: bool) -> std::result::Result<(), CoolifyError> {
    ...
    if detailed {
        ...
        for target_name in target_names {
            ...
            match api.get_services().await {
                ...
                Err(e) => {
                    tracing::warn!(
                        "No se pudo obtener estado de servicios para '{}': {e}",
                        target.name
                    );
    ...
    for site in &settings.sitios {
        ...
        println!(
            "{:<15} {:<35} {:<12} {:<25} {}",
            site.nombre, site.dominio, target_name, uuid_display, status
        );
    }
    ...
    if !settings.minecraft.is_empty() {
        ...
        for mc in &settings.minecraft {
            ...
            println!(
                "  {} ({}RAM, {}p) uuid={}",
                mc.server_name, mc.memory, mc.max_players, uuid
            );
        }
    }

    println!(
        "\nTotal: {} sitio(s), {} minecraft",
        settings.sitios.len(),
        settings.minecraft.len()
    );
    Ok(())
}
