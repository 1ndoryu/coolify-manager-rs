...
async fn fase_pull_tema(ctx: &CtxActualizacionTema<'_>) -> std::result::Result<(), CoolifyError> {
...
async fn fase_sincronizar_composer(
    ctx: &CtxActualizacionTema<'_>,
    antes: Option<String>,
...
async fn fase_ejecutar_migraciones(ctx: &CtxActualizacionTema<'_>) {
    let (ssh, container_id) = (ctx.ssh, ctx.container_id);
        ...
        Ok(pg_container) => {
            ...
            match (pg_user_res, pg_db_res) {
                (Ok(u), Ok(d)) => {
                    ...
                    } else if let Err(e) = super::theme_migrations::run_pending_migrations(
                        ssh,
                        container_id,
                        &pg_container,
                        ctx.theme_name,
...
async fn fase_aplicar_permisos(ctx: &CtxActualizacionTema<'_>) {
    let (ssh, container_id) = (ctx.ssh, ctx.container_id);
    let _ = docker::docker_exec(
        ssh,
        container_id,
...
async fn fase_escribir_php_ini(ctx: &CtxActualizacionTema<'_>) {
    let (ssh, container_id) = (ctx.ssh, ctx.container_id);
