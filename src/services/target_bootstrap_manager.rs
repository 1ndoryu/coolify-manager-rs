...
pub async fn install_coolify(target: &DeploymentTargetConfig) -> std::result::Result<InstallCoolifyReport, CoolifyError> {
    ...
    let os_name = ssh
        ...
        .trim()
        .to_string();
    let mut notes = vec![format!("SO detectado: {}", if os_name.is_empty() { "unknown" } else { &os_name })];
    ...
    let http_status = ssh
        .execute("bash -lc \"curl -fsS -o /dev/null -w '%{http_code}' http://127.0.0.1:8000 || true\"")
        .await?
        .stdout
        .trim()
        .to_string();
    if http_status == "200" || http_status == "302" {
        notes.push(format!("Coolify responde localmente con HTTP {}.", http_status));
    } else {
        notes.push(format!("Coolify aun no responde con 200/302 en localhost:8000 (actual: {}).", if http_status.is_empty() { "sin respuesta" } else { &http_status }));
    }

    notes.push("Despues de instalar hay que abrir la UI y crear el primer admin manualmente.".to_string());
