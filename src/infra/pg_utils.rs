...
pub fn parse_pg_credentials(
    ...
    let without_scheme = database_url
        ...
        .ok_or_else(|| {
            CoolifyError::Validation("DATABASE_URL no es una URL postgres válida".into())
        })?;
    ...
    let user = user_part.split(':').next().unwrap_or(user_part).to_string();
...
mod tests {
    ...
    fn test_parse_pg_credentials_postgresql_scheme() {
        let (user, db) = parse_pg_credentials("postgresql://app:pass@localhost/mydb").unwrap();
        assert_eq!(user, "app");
        assert_eq!(db, "mydb");
    }
